...
def train_segmentation_model(model, train_loader, val_loader, epochs=25, learning_rate=0.001, weight_decay=0.0, max_steps_per_epoch=100):
    ...
    for epoch in range(epochs):
        ...
        train_loss, train_iou = train_segmentation_epoch(
            model, train_loader, optimizer, criterion, device, max_steps_per_epoch)

        # Validation phase
        val_loss, val_iou = validate_segmentation_epoch(
...
def visualize_predictions(model, dataset, num_samples=6, save_path=None):
    ...
    with torch.no_grad():
        for i in range(num_samples):
            ...
            # Original image
            axes[0, i].imshow(img_np, cmap='gray' if len(
                img_np.shape) == 2 else None)
