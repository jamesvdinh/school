...
def floor_div(args):
    ...
    def calc(args, res):
        if args is nil:
            return res
        res //= args.first
        return calc(args.rest, res)
    return calc(args.rest, args.first)
...
def eval_and(expressions):
    ...
    if expressions is nil:
        return True
    first_eval = calc_eval(expressions.first)
    if first_eval is scheme_f:
        return False
...
def eval_define(expressions):
    ...
    bindings[sym] = calc_eval(expressions.rest.first)
    return sym
