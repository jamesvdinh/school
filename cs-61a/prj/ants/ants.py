...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        curr = self.place
        entr_transitions = 0
        while not curr.is_hive:
            if curr.bees and entr_transitions >= self.lower_bound and entr_transitions <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        bees = self.place.bees if self.place else []
        super().reduce_health(amount)
        for bee in list(bees):
            bee.reduce_health(amount if self.place else amount + self.damage)
        # END Problem 5
