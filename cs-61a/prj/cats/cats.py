...
def about(subject):
    ...
    def h(s):
        for word in subject:
            if word in [w.lower() for w in remove_punctuation(s).split()]:
                return True
        return False
    return h
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    def h(t, s):
        if not t or not s:
            return 0
        elif t[0].replace('\t', '') == s[0]:
            return 1 + h(t[1:], s[1:])
        else:
            return h(t[1:], s[1:])
    return h(typed_words, source_words) / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    closest_i = min(range(len(word_list)), key=lambda i: abs(
        diff_function(typed_word, word_list[i], limit)))
    return word_list[closest_i] if diff_function(typed_word, word_list[closest_i], limit) <= limit else typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    def h(t, s, lim):
        if lim > limit:
            ...
            return 1 + h(t[1:], s[1:], lim + 1)
        elif t[0] != s[0]:
            return 1 + h(t[1:], s[1:], lim + 1)
        else:
            return h(t[1:], s[1:], lim)
    return h(typed, source, 0)
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    def h(t, s):
        if t[:1] != s[:1] or not s:
            ...
            return 1 + h(t[1:], s[1:])
    progress = h(typed, source) / len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    times = [[i[j + 1] - i[j] for j in range(len(i) - 1)]
             for i in tpp]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    [4, 1, 6]
    """
    check_words_and_times(
        words_and_times)  # verify that the input is properly formed
    words, times = words_and_times['words'], words_and_times['times']
    player_indices = range(len(times))  # contains an *index* for each player
    word_indices = range(len(words))    # contains an *index* for each word
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    return [[words[j] for j in word_indices if min(player_indices, key=lambda i: times[i][j]) == p] for p in player_indices]
    # END PROBLEM 10
...
def check_words_and_times(words_and_times):
    ...
    """
    assert 'words' in words_and_times and 'times' in words_and_times and len(
        words_and_times) == 2
    words, times = words_and_times['words'], words_and_times['times']
...
def get_time(times, player_num, word_index):
    ...
    assert word_index < len(
        times[0]), f"word_index {word_index} outside of 0 to {num_words-1}"
    assert player_num < len(
        times), f"player_num {player_num} outside of 0 to {num_players-1}"
